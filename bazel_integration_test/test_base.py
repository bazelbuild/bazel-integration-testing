...
class TestBase(unittest.TestCase):
    ...
    def AssertExitCode(self, actual_exit_code, expected_exit_code,
                       stderr_lines):
        """Assert that `actual_exit_code` == `expected_exit_code`."""
        if actual_exit_code != expected_exit_code:
            self.fail('\n'.join([
                'Bazel exited with %d (expected %d), stderr:' %
    ...
    """
        if os.path.isabs(path) or '..' in path:
            raise ArgumentError(
                ('path="%s" may not be absolute and may not contain '
    ...
    """
        ...
        if os.path.exists(abspath):
            if os.path.isdir(abspath):
                return
            raise IOError('"%s" (%s) exists and is not a directory' % (path,
                                                                       abspath))
        os.makedirs(abspath)
    ...
    """
        ...
        if os.path.exists(abspath) and not os.path.isfile(abspath):
            raise IOError('"%s" (%s) exists and is not a file' % (path,
                                                                  abspath))
    ...
    """
        if not self._bazel:
            self.fail(
                "No version of bazel specified, please use SetBazelVersion.")
